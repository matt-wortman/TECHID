import { getSnapshotDetail, getSubmissionSnapshots } from '../../../actions'
...
function renderRepeatableTable(
  ...
  return (
    <Table wrapperClassName="neumorphic-table-container" className="neumorphic-table">
      <TableHeader>
        <TableRow>
          {columns.map((column) => (
...
export default async function SnapshotPage({ params }: SnapshotPageProps) {
  // First get all snapshots for this submission to find the latest one
  const snapshotsResult = await getSubmissionSnapshots(params.submissionId)
  ...
  const latestSnapshotId = snapshotsResult.snapshots[0].id
  const result = await getSnapshotDetail(latestSnapshotId)
  ...
  const scoringInputs = extractScoringInputs(responses as Record<string, unknown>)
  const computedScores = calculateAllScores(scoringInputs)
  const displayScores = {
    impactScore:
      (calculatedScores as Record<string, number> | null)?.impactScore ?? computedScores.impactScore,
    valueScore:
      (calculatedScores as Record<string, number> | null)?.valueScore ?? computedScores.valueScore,
    marketScore:
      (calculatedScores as Record<string, number> | null)?.marketScore ?? computedScores.marketScore,
    overallScore:
      (calculatedScores as Record<string, number> | null)?.overallScore ?? computedScores.overallScore,
  ...
  const marketSubCriteria = [
    ...
    },
    {
      ...
      description: describeScore(scoringCriteria.patientPopulation, scoringInputs.patientPopulationScore),
    },
    {
  ...
  const techMeta = technologyMeta as {
    techId?: string
      ...
      <div className="bg-blue-600 text-white py-3">
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-semibold">Historical Snapshot</span>
              <span className="mx-2">•</span>
              <span>Captured {formatDate(capturedAt)}</span>
              {capturedBy && (
                <>
                  <span className="mx-2">•</span>
                  <span>by {capturedBy}</span>
                </>
              )}
            </div>
      ...
      <div className="container mx-auto px-4 py-8 max-w-5xl space-y-6">
        ...
        {techMeta && (
          <Card className="bg-blue-50 border-blue-200 rounded-3xl">
            <CardContent className="pt-6">
              <h2 className="text-lg font-semibold text-blue-800 mb-3">Technology Details at Time of Snapshot</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                {techMeta.techId && (
                  <div>
                    <span className="text-blue-600 font-medium">Tech ID</span>
                    <p className="text-blue-900">{techMeta.techId}</p>
                  </div>
                )}
                {techMeta.technologyName && (
                  <div>
                    <span className="text-blue-600 font-medium">Technology</span>
                    <p className="text-blue-900">{techMeta.technologyName}</p>
                  </div>
                )}
                {techMeta.reviewerName && (
                  <div>
                    <span className="text-blue-600 font-medium">Reviewer</span>
                    <p className="text-blue-900">{techMeta.reviewerName}</p>
                  </div>
                )}
                {techMeta.currentStage && (
                  <div>
                    <span className="text-blue-600 font-medium">Stage</span>
                    <p className="text-blue-900">{techMeta.currentStage}</p>
                  </div>
                )}
              </div>
            </CardContent>
        ...
        <Card className="bg-[#e0e5ec] shadow-none border-0">
          <CardContent className="space-y-2">
            ...
            {template.description && <p className="text-[#6b7280]">{template.description}</p>}
            <div className="flex flex-wrap gap-3 text-sm text-[#6b7280]">
              <span>Version {templateVersion}</span>
              <span>Submitted {formatDate(submission.submittedAt)}</span>
              {submission.submittedBy && <span>Submitted by {submission.submittedBy}</span>}
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="space-y-6">
                  ...
                  <TableRow key={row.criteria} className="bg-blue-50">
                    <TableCell>
                      <div className="space-y-1">
                        ...
                        {row.description && <p className="text-xs text-[#6b7280]">{row.description}</p>}
                      </div>
                    </TableCell>
            ...
            <div className="grid md:grid-cols-3 gap-6">
                  ...
                  <div className="mt-3 space-y-3">
                    {marketSubCriteria.map((item) => (
                      <div key={item.label} className="flex items-start justify-between gap-4">
                        ...
                        <span className="text-sm font-semibold font-mono text-[#1f2937]">{item.score}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="rounded-2xl border-0 bg-[#f8fafc] p-5 space-y-4 [box-shadow:5px_5px_12px_rgba(163,177,198,0.25),-5px_-5px_12px_rgba(255,255,255,0.7)]">
                ...
                <div
                  className={`rounded-2xl px-4 py-3 text-center ${getRecommendationColorClasses(displayScores.recommendation)} bg-opacity-80 [box-shadow:4px_4px_10px_rgba(163,177,198,0.25),-4px_-4px_10px_rgba(255,255,255,0.7)]`}
                >
                  <p className="text-sm font-semibold uppercase tracking-wide">
                    {displayScores.recommendation}
                  </p>
                  <p className="text-xs text-[#1f2937] mt-1">{displayScores.recommendationText}</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        {sortedSections.map((section) => {
          ...
          return (
            <Card key={section.id} className="bg-[#e0e5ec] rounded-3xl shadow-none border-0">
              <CardHeader>
                <CardTitle className="text-2xl text-[#353535]">{section.title}</CardTitle>
                {section.description && <p className="text-sm text-[#6b7280]">{section.description}</p>}
              </CardHeader>
              <CardContent>
                <div className="space-y-6">
                  {questions.map((question) => {
                    const questionKey = question.dictionaryKey
                    const responseValue = (responses as Record<string, unknown>)[questionKey]
                    const repeatGroupValue =
                      (repeatGroups as Record<string, Record<string, unknown>[]>)[questionKey] ?? []
                    const isRepeatable =
